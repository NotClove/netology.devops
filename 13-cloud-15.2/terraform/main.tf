...
resource "yandex_storage_object" "thumbupcat" {
  access_key = yandex_iam_service_account_static_access_key.sa-static-key.access_key
  secret_key = yandex_iam_service_account_static_access_key.sa-static-key.secret_key
...
resource "yandex_compute_instance_group" "instance-group" {
  ...
  instance_template {
    ...
    metadata = {
      ...
      user-data = "#!/bin/bash\n cd /var/www/html\n echo \"<html><h1>kotik like this</h1><img src='https://${yandex_storage_bucket.vp-bucket.bucket_domain_name}/${yandex_storage_object.thumbupcat.key}'></html>\" > index.html"
    }
    labels = {
